...
synParmHat = lognfit(nonzeros(wtValsEx(wtValsEx>1)));
%wtValsEx = sort(wtValsEx, 'descend');
%wtValsEx =
%wtValsEx(uint32((0.01*length(wtValsEx)):(0.99*length(wtValsEx))));
[BinHeight, Edges] = histcounts(wtValsEx, logspace(-2, 2, 100), 'Normalization','pdf');
BinCenter = Edges(1:(length(Edges)-1)) + (diff(Edges)./2);
xs = logspace(log10(min(BinCenter)), log10(max(BinCenter)), 1000);
